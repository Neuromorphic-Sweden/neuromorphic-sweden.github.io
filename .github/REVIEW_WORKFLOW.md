...
Crawler proposals can first be selected in the local WYSIWYG queue at
`http://localhost:4321/preview/`: run the ignored Python service with `crawler_service.py review`
and start Astro using `LOCAL_REVIEW=true npm run dev -- --background`. Keep/Discard persists in
local SQLite without writing drafts into website source files. `publish-review` uploads only
kept records; pending and discarded material stays local. See README.md for startup details.

GitHub pull requests remain the final publication gate. The Python crawler, database, raw
pages and model prompts remain local on the Ubuntu server; only selected JSON records are pushed.

## Reviewer workflow

1. Open the crawler pull request and wait for `Validate proposed content` and the review deployment
   to pass.
2. Open `https://review.neuromorphic-sweden.se/preview/`. It renders the current review branch using the
...
6. When every remaining record is suitable, submit an approving GitHub review and merge the pull
   request into `main`. Use the **Conversation** tab, scroll to **Merge pull request** (or
   **Squash and merge**), and confirm. The merge is the publication decision.
...
```sh
npm ci
REVIEW_SITE=true npm run dev -- --background
```
